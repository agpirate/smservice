  ...
  export default {
    data() {
      return {
        ...
        roleOptions: ['admin', 'head', 'member', 'official'],
        userToDelete: null,
        editedUser: {
          id: '',
          firstname: '',
    ...
    methods: {
      ...
      async confirmDeleteUser() {
        ...
        if (uid) {
          try {
              await axios.delete('http://localhost:3000/user/profile?id='+uid);
            //await axios.delete(`http://localhost:8000/user/${uid}`);
          } catch (error) {
      ...
      async updateUser() {
        ...
        // const response = await axios.patch(`http://localhost:8000/user/${this.editedUser.id}`, this.editedUser);
      ...
      async createUser() {
        ...
        //const response = await axios.post('http://localhost:8000/user', this.newUser);
        const userData = this.newUser;
        console.log(userData)
        const response = await axios.post('http://localhost:3000/user/profile', userData);
