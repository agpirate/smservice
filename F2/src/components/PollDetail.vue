  ...
  <template v-slot:body="{ items }">
    <tbody>
      <tr v-for="(item, index) in items" :key="item.id">
        ...
        <td>
          <v-avatar size="40">
            <!-- <img :src="'http://localhost:8000/uploads/' + item.photo" alt="Contestant Photo"> -->

            <img :src="'http://localhost:3000/photos/' + item.photo" alt="Contestant Photo">
          </v-avatar>
        </td>
        <td>{{ item.age }}</td>
        <td>{{ item.code }}</td>
      ...
      <v-card-text>
        ...
        <div v-if="selectedContestant" class="contestant-details">
          <div class="avatar-container">
            <v-avatar size="200">
              <!-- <img :src="'http://localhost:8000/uploads/' + selectedContestant.photo" alt="Contestant Photo"> -->

              <img :src="'http://localhost:3000/photos/' + selectedContestant.photo" alt="Contestant Photo">
            </v-avatar>
          </div>
  ...
  methods: {
    ...
    async submitEditedContestant() {
      try {
        ...
        const response = await axios.put(`http://localhost:3000/contestant/contestants?id=${this.editedContestant.id}`, this.editedContestant);
    ...
    async submitContestant() {
      try {
        ...
        // let response = await axios.get('http://localhost/pollcontestant/pll')
          // console.log(response)
          try {
            // let response = await axios.post('http://localhost:8000/pollcontestant/pll', formData, {

            let response = await axios.post('http://localhost:3000/contestant/contestants', formData, {
              headers: {
    ...
    async fetchPoll(pollId) {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/pollprogram/pollprograms/?id=${pollId}`);
    ...
    async fetchContestants(pollId) {
      try {
        ...
        // const response = await axios.get(`http://localhost:8000/pollcontestant/pll?pid=${pollId}`)

        const response = await axios.get(`http://localhost:3000/contestant/contestants?pid=${pollId}`)
        // const response_score = await axios.get(`http://localhost:8000/panswer?pid=${pollId}`)
        console.log(response.data)
        if(response.data){
          this.contestants = response.data;
        }
        // response_score = await axios.get(`http://localhost:8000/panswer?pid=${pollId}`)
