...
export default {
  ...
  methods: {
    async fetchPoll(pollId) {
      try {
        const response = await axios.get(`http://localhost:3000/feedback/feedbacks?id=${pollId}`);
    ...
    async fetchContestants(pollId) {
      try {
        const response = await axios.get(`http://localhost:3000/response/fresponse?fid=${pollId}`);
