<template>
  <div class="poll-detail-container">
    <h1 style="color: teal;">Staff Information:</h1>
    
    <div class="contestants">
      <h3 style="color:teal">Staff:</h3>
      <div class="search-and-buttons"> <!-- Flex container for search field and buttons -->
        <div class="buttons">
          <v-btn color="primary" @click="openDialog1()">Create Staff</v-btn>
  ...
  <template v-slot:body="{ items }">
    ...
    <v-dialog v-model="dialogOpen" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-textarea v-model="newPhone" label="Add New Phone Number"></v-textarea>
    ...
    <template v-if="snackbarOpen">
     ...
     <!-- Dialog for composing and sending message -->
     <v-dialog v-model="dialogOpen1" max-width="500px">
      <v-card>
        <v-card-title>Create Staff</v-card-title>
        <v-card-text>
          <v-text-field v-model="fname" label="Full Name" ></v-text-field>
          <v-select
          v-model="department"
          ...
          <!-- <v-select v-if="departmentOptions" v-model="department" placeholder="Department" >
              <option value="">Select Department</option>
              <option v-for="department1 in departmentOptions" :key="department1" :value="department">{{ department1 }}</option>
          </v-select> -->
                    <!-- Input field for adding new phone numbers -->
        ...
        <v-card-actions>
          <v-btn color="primary" @click="saveStaff()">Send</v-btn>
          <v-btn color="primary" @click="dialogOpen1 = false">Cancel</v-btn>
        </v-card-actions>
         <!-- Snackbar message inside dialog -->
    <template v-if="snackbarOpen">
      <v-snackbar v-model="snackbarOpen" :timeout="snackbarTimeout" :color="snackbarColor">
...
export default {
  ...
  methods: {
    async fetchClients() {
      try {
        const response = await axios.get(`http://192.168.7.5:3000/client/clientf`);
    ...
    addNewPhone() {
      ...
      if (this.newPhone.trim() !== '') {
        ...
        const newPhones = this.newPhone.split(/,(?=\s*\+?\d{1,3}?[-.\s]?\(?\d{1,4}?\)?[-.\s]?\d{1,4}[-.\s]?\d{1,9})/)
            .map((phone) => phone.replace(/\s+/g, ''))
            .map((phone) => phone.trim()).filter((phone) =>phone);
        newPhones.forEach((phone)=> {
          if(!this.selectedClients.includes(phone)){
            this.selectedClients.push(phone)
    ...
    async sendMessage() {
      ...
      // console.log(sms)
      if(sms.phones){
        const response = await axios.post(`http://192.168.7.5:3000/send/sms`,sms);
    ...
    async saveStaff() {
      ...
      let staff = {}
      staff.name = this.fname;
      staff.department = this.department;
      staff.phone = this.phone
      // console.log(sms)
      if(staff.name && staff.department && staff.phone){
        console.log("Saving staff")
        const response = await axios.post(`http://192.168.7.5:3000/canalfeed/canalfeed`,staff);
        console.log(response.data)
        // Show snackbar with success message
        this.snackbarMessage = 'Staff created successfully!';
        this.snackbarColor = 'success';
        this.snackbarOpen = true;
        this.dialogOpen =false
      } else {
        // Show snackbar with error message
        this.snackbarMessage = 'Error creating user. Please check the required fields and try again later.';
        this.snackbarColor = 'error';
        this.snackbarOpen = true;
      }
      // this.dialogOpen = false;
