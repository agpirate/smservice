<template>
  <div>
    <h2 class="table-title">FeedBack</h2>
    <button @click="openCreateDialog" class="create-button" v-if="currentRole=='admin' || currentRole=='ict'">Create New FeedBack</button>
    ...
    <div class="filters">
      ...
      <input v-model="filterProgramIdCode" class="filter-input" placeholder="Search Feedback code/fCode">
    ...
    <table class="poll-table">
      ...
      <tbody>
        <tr v-for="(poll, index) in paginatedPolls" :key="poll.id">
          <td>{{ index + 1 }}</td>
          <router-link :to="`/feedbacks/${poll.id}`">
            <td>{{ poll.summary }}</td>
          </router-link>
...
<dialog ref="editDialog" class="modal">
      ...
      <form @submit.prevent="updatePoll">
        ...
        <input label="Program ID/code" v-model="editedPoll.fcode" placeholder="Program ID/Code">
        <div>
  ...
  <form @submit.prevent="createPoll">
    ...
    <input label="Feedback code/fCode" v-model="newPoll.fcode" @input="validateProgramId" class="filter-input"  placeholder="Feedback code/fCode">
...
export default {
  ...
  async mounted() {
    ...
    setInterval(this.fetchPolls, 1000); // Call fetchPolls every 1 second
  },
  watch: {
  ...
  methods: {
    validateProgramId() {
      if (this.newPoll.fcode.length !== 2 || this.newPoll.fcode[0] !== 'f'  ) {
        // Display an error message or take appropriate action
        console.log("feedback ID/fcode should have 2 characters and start with 'f' or 'F'");
      }
    },
  ...
  if (pid) {
    try {
      ...
      const response = await axios.delete('http://192.168.7.5:3000/feedback/feedbacks?id='+pid); 
  ...
  this.$refs.confirmationDialog.close();
    ...
    editPoll(poll) {
     ...
     async updatePoll() {
      ...
      let feedback=this.editedPoll;
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
     const response = await axios.put('http://192.168.7.5:3000/feedback/feedbacks',feedback); // Fetch data from JSON file
    ...
    async createPoll() {
      ...
      if (this.newPoll.fcode.length !== 2 || this.newPoll.fcode[0] !== 'f') {
        this.programIdError = "feedback ID/fcode should have 2 characters and start with 'f' or 'F'";
        return; // Exit the method if validation fails
      }
      // Save the new poll to local storage
     // let polls = JSON.parse(localStorage.getItem('myPollData')) || [];
      //onsole.log(this.newPoll)
      //polls.push(this.newPoll);
      let feedback={
          creator_id:localStorage.getItem('id'),
          summary:this.newPoll.summary,
          fcode:this.newPoll.fcode,
    ...
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
     const response = await axios.post('http://192.168.7.5:3000/feedback/feedbacks',feedback); // Fetch data from JSON file
    ...
    async fetchPolls() {
      try {
        ...
        // const response = await axios.get('http://192.168.7.5:8000/poll', {
          const response = await axios.get('http://192.168.7.5:3000/feedback/feedbacks', {
  ...
  computed: {
    ...
    columns() {
      return ['Number', 'Summary','Department', 'Creator ID', 'Feedback Code/fCode', 'Schedule Start', 'Schedule End','Action'];
    },
    filteredPollList() {
