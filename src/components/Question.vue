<template>
  <div>
    <h2 class="table-title">Questions</h2>
    <button @click="openCreateDialog" class="create-button" v-if="currentRole=='admin' || currentRole=='ict'">Create New Question</button>
    ...
    <table class="poll-table">
      ...
      <tbody>
        <tr v-for="(poll, index) in paginatedPolls" :key="poll.id">
          <td>{{ index + 1 }}</td>
          <router-link :to="`/questions/${poll.id}`">
            <td>{{ poll.summary }}</td>
          </router-link>
...
<dialog ref="editDialog" class="modal">
      ...
      <form @submit.prevent="updatePoll">
        ...
        <v-textarea v-model="editedPoll.summary" label="Summary" class="custom-textarea"></v-textarea>
        <select v-model="editedPoll.department" placeholder="Department" class="select-input">
            <option value="">Select Department</option>
            <option v-for="department in departmentOptions" :key="department" :value="department">{{ department }}</option>
       </select>
        <!-- <input label="creator_id" v-model="editedPoll.creator_id" placeholder="Creator ID"> -->
        <input label="Program ID/code" v-model="editedPoll.qcode" placeholder="Program ID/Code">
        <v-textarea v-model="editedPoll.answer" label="Answer" class="custom-textarea"></v-textarea>
  ...
  <form @submit.prevent="createPoll">
    ...
    <v-textarea v-model="newPoll.summary" label="Summary" class="custom-textarea"></v-textarea>
    <!-- <input label="creator_id" v-model="newPoll.creator_id" :placeholder="newPoll.creator_id" readonly> -->
    <select v-model="newPoll.department" placeholder="Department" class="select-input">
            <option value="">Select Department</option>
            <option v-for="department in departmentOptions" :key="department" :value="department">{{ department }}</option>
    </select>
    <input label="Question code/qcode" v-model="newPoll.qcode" @input="validateProgramId" class="filter-input" placeholder="Question code/ qcode">
    <span v-if="programIdError" class="error-message">{{ programIdError }}</span>      
    <v-textarea v-model="newPoll.answer" label="Answer" class="custom-textarea"></v-textarea>
...
export default {
  ...
  async mounted() {
    ...
    //this.interval =  setInterval(this.fetchPolls(), 3000); // Call fetchPolls every 1 second
  },
  beforeUnmount() {
  ...
  methods: {
    validateProgramId() {
      if (this.newPoll.qcode.length !== 2 || this.newPoll.qcode[0] !== 'q') {
        // Display an error message or take appropriate action
        console.log("question ID/Qcode should have 2 characters and start with 'q'");
      }
    },
  ...
  if (pid) {
    try {
      //await axios.delete(`http://192.168.7.5:8000/poll/${pid}`);
      const response = await axios.delete('http://192.168.7.5:3000/question/questions?id='+pid); 
  ...
  this.$refs.confirmationDialog.close();
    ...
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
      await axios.put('http://192.168.7.5:3000/question/questions',poll); // Fetch data from JSON file
        // this.polls.push(response.data)
        const index = this.polls.findIndex(q => q.id === this.editedPoll.id);
        if (index !== -1) {
          this.polls[index] = { ...this.editedPoll };
          this.$set(this.polls, index, { ...this.editedPoll });  

        }
        // localStorage.setItem('myPollData', JSON.stringify(this.polls));
    ...
    async createPoll() {
      ...
      if (this.newPoll.qcode.length !== 2 || this.newPoll.qcode[0] !== 'q') {
        this.programIdError = "question ID/qcode should have 2 characters and start with 'q'";
      ...
      let poll={
          ...
          qcode:this.newPoll.qcode,
          answer:this.newPoll.answer,
          department:this.newPoll.department,
          schedule:{
    ...
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
     const response = await axios.post('http://192.168.7.5:3000/question/questions',poll); // Fetch data from JSON file
    ...
    async fetchPolls() {
      try {
        ...
        // const response = await axios.get('http://192.168.7.5:8000/poll', {
          ...
          // const response = await axios.get('http://192.168.7.5:8000/poll'); // Fetch data from JSON file
          // this.polls = '';
    ...
    getColumnValue(poll, column) {
      switch (column) {
        ...
        case 'qcode':
          return poll.qcode.toLowerCase();
        case 'answer':
          return poll.answer.toLowerCase();
        case 'schedule start':
  ...
  computed: {
    ...
    columns() {
      return ['Number', 'Summary', 'Creator ID', 'Question Code/qcode','Answer', 'Schedule Start', 'Schedule End','Action'];
    },
    filteredPollList() {
      ...
      if (this.filterProgramIdCode) {
        filteredPolls = filteredPolls.filter(poll => poll.qcode.toLowerCase().includes(this.filterProgramIdCode.toLowerCase()));
      }
