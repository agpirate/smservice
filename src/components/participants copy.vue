  ...
  <template v-slot:body="{ items }">
    ...
    <v-dialog v-model="dialogOpen" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn color="primary" @click="sendMessage()" :disabled="messageError || !message">Send</v-btn>
...
export default {
  ...
  try {
    ...
    // this.$set(this.snackbar, 'message', 'Message sent successfully');
    // this.$set(this.snackbar, 'color', 'success');
  } catch (error) {
    console.error('Error sending message:', error);
    // Show error message in snackbar
    // this.$set(this.snackbar, 'show', true);
    // this.$set(this.snackbar, 'message', 'Failed to send message');
    // this.$set(this.snackbar, 'color', 'error');
  }

  // Close dialog after attempting to send message
  this.dialogOpen = false;

  // Reset selected clients and message
  this.selectedClients = [];
  this.message = '';
}
