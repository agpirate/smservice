<template>
  <div>
    ...
    <button @click="openCreateDialog" class="create-button" v-if="currentRole=='admin' || currentRole=='ict'">Create New Poll</button>
...
export default {
  ...
  if (pid) {
    try {
      ...
      const response = await axios.delete('http://192.168.7.5:3000/pollprogram/pollprograms?id='+pid); 
  ...
  this.$refs.confirmationDialog.close();
    ...
    editPoll(poll) {
     ...
     async updatePoll() {
      ...
      console.length
      if (!([2,3].includes(this.editedPoll.programId.length)) || !(['p','t'].includes(this.editedPoll.programId[0]))) {
        alert("hsdffhsd",this.editedPoll.programId.length,this.editedPoll.programId[0])
    ...
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
     const response = await axios.put('http://192.168.7.5:3000/pollprogram/pollprograms',poll); // Fetch data from JSON file
    ...
    async createPoll() {
      // Perform validation before creating a new poll
      // Perform validation before creating a new poll
      if (!([2,3].includes(this.newPoll.programIdcode.length)) || !(['p','t'].includes(this.newPoll.programIdcode[0]))) {
        this.programIdError = "Program ID/Code should have 2 characters and start with 'p'";
        return; // Exit the method if validation fails
      }
    ...
    //  const response = await axios.post('http://192.168.7.5:8000/poll',poll); // Fetch data from JSON file
     const response = await axios.post('http://192.168.7.5:3000/pollprogram/pollprograms',poll); // Fetch data from JSON file
    ...
    async fetchPolls() {
      try {
        ...
        // const response = await axios.get('http://192.168.7.5:8000/poll', {
          const response = await axios.get('http://192.168.7.5:3000/pollprogram/pollprograms', {
